...
export const getAllAlbums = tryCatch(async (req: Request, res: Response, next: NextFunction) => {
    try {
...
export const getAllSongs = tryCatch(async (req: Request, res: Response, next: NextFunction) => {
    try {
        let songs;
        const CACHE_EXPIRY=1800;
        if(redisClient.isReady){
...
export const getAllSongOfAlbum = tryCatch(async (req: Request, res: Response, next: NextFunction) => {
    try {
        const { id } = req.params;
        const CACHE_EXPIRY=1800;
        let album,songs;
...
export const getSingleSong = tryCatch(async (req: Request, res: Response, next: NextFunction) => {
    const { id } = req.params;
